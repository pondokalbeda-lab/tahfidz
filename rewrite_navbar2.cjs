      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2.5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-8 h-8 rounded-lg bg-white p-0.5 shadow-sm flex items-center justify-center hidden sm:flex">
              <div className="w-full h-full bg-slate-50 rounded flex items-center justify-center">
                <span className="text-emerald-800 font-bold text-sm">AF</span>
              </div>
            </div>
            <div className="flex flex-col">
              <h1 className="text-base sm:text-lg font-bold tracking-tight text-white leading-tight flex items-center gap-1.5 truncate max-w-[200px] sm:max-w-md">
                <Building2 className="w-4 h-4 text-emerald-300 sm:hidden flex-shrink-0" />
                <span className="truncate">{pesantrenInfo.nama || 'Al-Furqon Tahfidz'}</span>
              </h1>
              <p className="text-[10px] sm:text-xs text-emerald-200 opacity-90 font-medium mt-0.5 hidden sm:block">
          ...
          <div className="flex items-center space-x-1 sm:space-x-3">
            ...
            <button
              ...
              title="Refresh Data"
              className="p-1.5 sm:p-2 hover:bg-emerald-700 rounded-lg transition-colors"
            >
            ...
            <button
              ...
              className="p-1.5 sm:p-2 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg border border-emerald-500/50 transition-colors flex items-center justify-center ml-1"
            >
        ...
        <div className="flex items-center justify-between mt-2 pt-2 border-t border-emerald-700/50">
           <span className="text-[11px] sm:text-xs text-emerald-200 font-medium">TA {pesantrenInfo.tahunAjaran} ({pesantrenInfo.semester})</span>
           <div className="flex items-center space-x-1.5 bg-emerald-900/50 px-2 py-1 rounded border border-emerald-700/50 text-emerald-100">
              <Calendar className="w-3.5 h-3.5" />
              <input
                type="date"
                value={selectedDate}
                onChange={(e) => setSelectedDate(e.target.value)}
                className="bg-transparent border-none text-[11px] sm:text-xs text-white focus:outline-none cursor-pointer max-w-[110px]"
              />
            </div>
        </div>
      </div>
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <nav className="flex flex-col gap-1.5">
                <button
                  ...
                  className={\`flex items-center space-x-3 px-3 py-3 rounded-xl text-sm font-semibold transition-all w-full \${
                    activeTab === 'dashboard'
